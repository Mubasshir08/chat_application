...
const Sidebar = () => {
  ...
  useEffect(() => {
    const searchedUser = searchInput !== '' && searchInput !== " " && otherUsers?.filter(user => (user.userName.toLowerCase().includes(searchInput.toLowerCase())))
  ...
  return (
    <div className='w-1/3 border-r-2 border-gray-700 overflow-x-auto flex flex-col'>
      <form className='sticky top-3 mt-5 text-center' onSubmit={(e) => e.preventDefault()}>
        <input type="text" className='input input-sm w-3/4' value={searchInput} onChange={(e) => setSearchInput(e.target.value)}/>
        <button type='submit' className='absolute right-12 top-0.5'>
          <IoSearchCircle className='text-black' size="30px" />
        </button>
      </form>
      <div className="divider"></div>
        <UsersDisplay searchedUser={searchResult}/>
